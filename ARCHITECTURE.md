...
| `/innerspace` | `Views/InnerSpace.tsx` | Portfolio |
| `/terminal` | `Views/The_Terminal/Terminal_Home.tsx` | Terminal |

The two layout components in `src/Components/UI/` render an `<Outlet />` without top navigation; the portfolio layout also renders a footer. Both layouts use a black background. `BbsPageFrame` surrounds each layout's route content while the footer remains outside it. The frame owns the outer ANSI border animation. On the home page, `AsciiBanner` reports its active style and frame to that border so both use the same glyph and color rules from `asciiFrames.ts`; without a banner, the border animates independently. `src/Views/` composes page content. Reusable portfolio cards and project presentations live in `src/Components/Portfolio/`; authentication and profile controls live under `src/Components/Auth/` and `src/Components/UserData/`. The home page renders `AsciiBanner` above its directly composed project cards. The former presentation card and its optional profile picture are no longer loaded. `src/Components/Portfolio/Cards/ProjectsCard.tsx` is a separate API-backed project carousel, not the home page's current project list.
...
On the home page, `InnerSpaceMessenger` reveals a link at the bottom of the scrollable page. The `/innerspace` view shows a static teaser scene and caption. Both pixel-art images are bundled from `src/Assets/`.
